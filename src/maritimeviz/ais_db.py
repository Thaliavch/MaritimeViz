...
#         for query in DATABASE_TABLE_CREATION_QUERIES:
...
class AISDatabase:
    ...
    # TODO(Thalia): This should also be implemented per class
    def search(self,
               mmsi: Optional[Union[int, List[int]]] = None,
        ...
        try:
            ...
            if mmsi:
                if isinstance(mmsi, int):
                    ...
                    query += " AND mmsi IN (" + ", ".join(["?"] * len(mmsi)) + ")"
                    params.extend(mmsi)
                else:
                    raise ValueError("MMSI must be an integer or a list of integers.")
            # Date and other filters would be added here
            logger.info(f"Executing query: {query} with params: {params}")
            df = conn.execute(query, params).fetchdf()
            if df.empty:
    ...
    def get_geojson(self, mmsi: Optional[int] = None, start_date: Optional[str] = None,
                    end_date: Optional[str] = None, polygon_bounds: Optional[str] = None) -> dict:
        try:
            gdf = self.search(mmsi, start_date=start_date, end_date=end_date, polygon_bounds=polygon_bounds)
            if gdf.empty:
                logger.info(f"No AIS data found for {mmsi}")
                return {}
            # Ensure datetime is JSON serializable if applicable
    ...
    # Other export methods like get_csv, get_parquet, etc. would be here.
...
class BaseMessageProcessor:
    ...
    def _process_chunk(self, chunk: list, msg_type: str):
        """
